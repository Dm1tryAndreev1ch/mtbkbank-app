...
const CATEGORIES = [
  { id: 'mobile', icon: 'phone-iphone', title: 'Мобильная связь', desc: 'A1, МТС, life:)', color: '#4F8EF7',
    services: [
      { name: 'A1 (velcom)', icon: 'signal-cellular-alt' },
      { name: 'МТС', icon: 'cell-tower' },
      { name: 'life:)', icon: 'phone-android' },
    ]},
  { id: 'utilities', icon: 'home', title: 'ЖКУ и коммуналка', desc: 'Свет, вода, газ, отопление', color: '#f59e0b',
    services: [
  ...
  { id: 'internet', icon: 'wifi', title: 'Интернет и ТВ', desc: 'Провайдеры, подписки', color: '#0ea5e9',
    services: [
      { name: 'Белтелеком', icon: 'router' },
      { name: 'A1 Интернет', icon: 'language' },
      { name: 'МТС Домашний', icon: 'tv' },
    ]},
  { id: 'fines', icon: 'gavel', title: 'Штрафы и налоги', desc: 'ГАИ, налоговая', color: '#ef4444',
    services: [
      { name: 'Штрафы ГАИ', icon: 'directions-car' },
      { name: 'Налоги', icon: 'receipt' },
      { name: 'Госпошлины', icon: 'account-balance' },
    ]},
  { id: 'education', icon: 'school', title: 'Образование', desc: 'Школы, вузы, курсы', color: '#9333EA',
    services: [
...
export default function PaymentScreen() {
  ...
  const filtered = search
    ? CATEGORIES.filter(c => c.title.toLowerCase().includes(search.toLowerCase()) || c.desc.toLowerCase().includes(search.toLowerCase()))
    : CATEGORIES;
  ...
  const handlePay = async () => {
    ...
    setLoading(true);
    try {
      await api.makePayment({ accountId: mainAcc.id, amount: amt, categoryId: selected?.id, merchant: service?.name || selected?.title });
      await loadAccounts();
      Alert.alert('Успешно', `Оплата ${formatMoney(amt)} проведена`, [{ text: 'OK', onPress: () => router.back() }]);
    } catch {
      Alert.alert('Успешно', `Оплата ${formatMoney(amt)} проведена (демо)`, [{ text: 'OK', onPress: () => router.back() }]);
  ...
  const headerTitle = service ? service.name : selected ? selected.title : 'Платежи';
      ...
      {!selected ? (
        <ScrollView contentContainerStyle={s.list} keyboardShouldPersistTaps="handled">
          <View style={s.searchBar}>
            ...
            <TextInput style={s.searchInput} placeholder="Найти услугу..." placeholderTextColor={colors.outlineVariant} value={search} onChangeText={setSearch} />
          </View>
          ...
          {filtered.length === 0 && <Text style={s.empty}>Ничего не найдено</Text>}
        </ScrollView>
      ) : !service ? (
